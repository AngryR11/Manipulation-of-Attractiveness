...
2. Introduced the parameter k, which indicates the number of colleges that each student finds acceptable

3. Introduced as an input colleges' ex-post preferences that take into account how well students rank the college (students who give a low priority to colleges are not as well ranked as those who give the college a higher priority), thus allowing us to implement the AR mechanism.
")
...
iaa <- function(nStudents=ncol(s.prefs), nColleges=ncol(c.prefs), nSlots=rep(1,nColleges), s.prefs=NULL, c.prefs=NULL,c.real_prefs=NULL, acceptance="deferred", short_match = TRUE,k_bar=nColleges){
  #List to store all students who proposed to each college
  ...
  while(min(s.hist[s.singles]) < k_bar){  # there are as many rounds as maximal preference orders
    ...
    if(length(temp.singles)==0){ # if unassigned students have used up all their offers: stop
      return(finish(s.prefs,c.prefs,c.real_prefs,iter,c.hist,s.singles,lists_ranking_of_all_students_who_proposed,short_match,nSlots))
    }
    ...
    for(j in approached){
      ...
      lists_ranking_of_all_students_who_proposed[[j]] <- c(lists_ranking_of_all_students_who_proposed[[j]], which(c.real_prefs[,j] %in% all_proposers))#UPDATED
  ...
  return(finish(s.prefs,c.prefs,c.real_prefs,iter,c.hist,s.singles,lists_ranking_of_all_students_who_proposed,short_match,nSlots))#added as intermediate output lists_ranking_of_all_students_who_proposed
}


# To Sum up and format the output
finish <- function(s.prefs,c.prefs,c.real_prefs,iter,c.hist,s.singles,lists_ranking_of_all_students_who_proposed,short_match,nSlots){
  ...
  if(short_match == FALSE){
    return(list(s.prefs=s.prefs,c.prefs=c.prefs,c.real_prefs,iterations=iter,matchings=edgefun(x=c.hist),singles=s.singles))
  }
  else {
    # Format matching
    ...
    for(j in 1:ncol(c.prefs)){
      if(j %in% matching$college){
        ranking_matched_students=sort(which(c.real_prefs[,j]%in%matching$student[matching$college==j]))#MAIN CHANGE from BOSTON
        if(length(ranking_matched_students)==nSlots[j]){#If a college managed to fill all of its vacancies, let's see if it could have gotten something better:
          ...
          if(ordered_lists_ranking_of_all_students_who_proposed[nSlots[j]]<ranking_matched_students[nSlots[j]]){#if the student could have gotten something better by rendering itself unacceptable to some student:
    ...
    return(list(s.prefs=s.prefs,prop_benefits_misreport=sum(benefits_from_manipulating_attractiveness)/length(benefits_from_manipulating_attractiveness),c.prefs=c.prefs,c.real_prefs=c.real_prefs,iterations=iter,matchings=matching,singles=s.singles, free_cap = free_caps))#=benefits_from_manipulating_attractiveness
  }
}
...
## Boston mechanism
 iaa(s.prefs = s.prefs, c.prefs = c.prefs, c.real_prefs=c.prefs, nSlots = nSlots,k=3)
#===========================================
 ...
 generate_random_preferences <- function(phi, m, q,theta) {
   ...
   # Adjust college preferences based on students' top choices
   c_ex_post_prefs <- matrix(NA, nrow = m, ncol = n_students)
   
   for (college in 1:m) {
     # Students who listed the college as one of their top 2 choices
     top_students <- which(s_prefs[, 1] == college | s_prefs[, 2] == college)
     
     # Students who did not list the college as a top 2 choice
     other_students <- setdiff(1:n_students, top_students)
     
     # Combine top students and other students based on original preferences
     priority_students <- c(
       top_students[order(match(top_students, c_prefs[, college]))],
       other_students[order(match(other_students, c_prefs[, college]))]
     )
     
     # Store the adjusted preferences
     c_ex_post_prefs[college, ] <- priority_students
   }
   ...
   return(list(
     ...
     c_prefs = c_prefs,
     n_slots = n_slots,
     c_ex_post_prefs=c_ex_post_prefs
   ))
 }
 ...
 run_simulation <- function(phi, theta, sample_size, m, q, k) {
   ...
   for (s in 1:sample_size) {
     ...
     c.prefs=t(random_prefs$c_ex_post_prefs) 
     c.real_prefs=(random_prefs$c_prefs)
     nSlots <- random_prefs$n_slots
     
     # Run the top trading cycles algorithm
     result <- iaa(s.prefs = s.prefs, c.prefs = c.prefs, c.real_prefs=c.real_prefs,nColleges = ncol(c.prefs), nSlots=nSlots, k_bar = k)
 ...
 clusterExport(cl, c(
   "generate_random_preferences", "iaa", 
   "finish", "m", "q", "k", "sample_size", "run_simulation","edgefun","proposer_better"
 ))
...
#Export results to csv (in case one wants to format the layout of the plot later)
write.csv(proportion_misreport, file='proportion_misreport_AR_parallel_computing_300.csv',row.names=FALSE)   

 
#Plotting results:
 for(l in 1:length(theta_grid)){
   if(l==1){
     pdf('incentives_manipulate_AR_sample_size_parallel_300.pdf',width=7,height=5)
